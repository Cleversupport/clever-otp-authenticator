...
jQuery(document).ready(function($) {
	...
	var procesRequestOTPCode = function() {
		var form = $('#otpa_otp_form'),
			data = {
				nonce   : form.find('#otpa_nonce').val(),
				handler : 'request_otp_code',
				otpType : form.data('otp_form_type'),
				payload : {
					identifier: form.find('#otpa_id').val()
				}
			};

		form.find('#otpa_result').html('');

		$.post(OTPA.otpa_api_url, data, function(response) {

			if (!response.success && response.data && response.data.message) {
				form.find('#otpa_result').html(response.data.message);
			} else if (response.data && response.data.message) {
				form.find('#otpa_result').html(response.data.message);
			} else {
				form.find('#otpa_result').html(form.find('#otpa_unknown_result').html());
			}
		}).fail(function() {
			form.find('#otpa_result').html(form.find('#otpa_unknown_result').html());
		});
	...
	$('#otpa_id_widget').on('change', function() {
		$(this).closest('#otpa_otp_form').find('#otpa_id').val($(this).val());
	});
	...
	$('#otpa_code_widget').on('change', function() {
		...
		var form = $(this).closest('#otpa_otp_form');

		form.find('#otpa_code').val( code );

		if (0 < code.length) {
			form.find('#otpa_submit').removeAttr('disabled');
		} else {
			form.find('#otpa_submit').attr('disabled', 'disabled');
		}
	});
	...
	$('#otpa_set_id_input').on('keyup', function() {
		...
		form.find('#otpa_id').val($(this).val());
	...
	$('#otpa_submit').on('click', function(e) {
		...
		var button = $(this),
			form   = button.closest('#otpa_otp_form'),
			data   = {
				nonce:   form.find('#otpa_nonce').val(),
