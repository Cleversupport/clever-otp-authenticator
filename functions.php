 ...
 * - If user meta 'otpa_set_otp_redirect' exists, use it (set during login/identifier).
 * - Else if request contains 'redirect_to', use it (sanitized and internal only).
 * - Else allow site-wide default via filter 'otpa_custom_default_redirect', falling back to home_url().
 */
add_filter('otpa_otp_form_redirect_url', function($url, $user){
	$redirect = '';
	if ( $user instanceof WP_User ) {
		$meta = get_user_meta( $user->ID, 'otpa_set_otp_redirect', true );
		if ( !empty($meta) ) {
			$redirect = $meta;
			// Clean it up so it won't persist for next logins
			delete_user_meta( $user->ID, 'otpa_set_otp_redirect' );
		}
	}
	if ( empty($redirect) && isset($_REQUEST['redirect_to']) ) {
		$maybe = esc_url_raw( wp_unslash( $_REQUEST['redirect_to'] ) );
		// Only allow internal redirects for safety
		if ( $maybe && 0 === strpos( $maybe, home_url('/') ) ) {
			$redirect = $maybe;
		}
	}
	if ( empty($redirect) ) {
		$default = apply_filters('otpa_custom_default_redirect', '');
		$redirect = $default ? $default : home_url();
	}
	return $redirect;
}, 10, 2);
