...
* **Customizable for developers:** developers can add their own gateways or add custom One-Time Password modes using action and filter hooks, and more - see the [developers documentation](https://github.com/Cleversupport/clever-otp-authenticator).
* **Integration-friendly:** specific integration with Ultimate Member and WooCommerce is included by default ; developers can easily plug into OTP Authenticator with a multitude of functions, filter hooks and action hooks - see the [developers documentation](https://github.com/Cleversupport/clever-otp-authenticator) - contributions to integrations are welcome.
...
OTP Authenticator is regularly updated, and bug reports are welcome, preferably on [Github](https://github.com/Cleversupport/clever-otp-authenticator/issues), especially for advanced troubleshooting.
...
More help can be found on <a href="https://github.com/Cleversupport/clever-otp-authenticator/issues">GitHub Issues</a> for troubleshooting, integration, and feature requests.
...
* Restore the Passwordless Authentication card styling for the full OTP page and embedded shortcode, including reliable form visibility when the logo cannot load.
...
* Reuse the shared OTP form renderer for the passwordless route and shortcode without adding another login process.

= 1.0.8 =
* Change Hide Login blocking to redirect direct wp-login.php and logged-out wp-admin requests to the site home page while keeping the public login slug redirecting to the passwordless login page.

= 1.0.7 =
* Fix Hide Login direct wp-login.php detection so enabled sites block the default login screen and show the site 404 page.
...
= 1.2.0 =
* Marked as compatible with WordPress 6.7 and PHP 8.3.
* Added support for redirect URL after successful OTP verification.
  - Honors the standard `redirect_to` parameter on login.
  - Preserves the requested redirect during OTP/identifier steps.
  - New filter `otpa_custom_default_redirect` to set a site-wide default.
